...
def add_to_words(user_id, word, correct=False):
    ...
    with Session(engine) as session:
        statement = select(Word).where(Word.word == lowered_word).where(Word.user_id == user_id)
        if session.execute(statement).scalar():
            ...
            session.commit()
            return
        word = Word(word=lowered_word, user_id=user_id, correct_guesses=int(correct), last_used=datetime.datetime.utcnow())
        session.add(word)
        session.commit()
...
def recall_word(message, tele_bot, gpt_api):
    ...
    with Session(engine) as session:
        ...
        if not user:
            tele_bot.send_message(message.chat.id, "You have not started using the bot yet. Please start using the bot by clicking /start.")
            return
        statement = select(Word).where(Word.user_id == user_id).order_by(Word.last_used).limit(1)
        word = session.execute(statement).scalar()
        if not word:
            tele_bot.send_message(message.chat.id, "You don't have any words to recall.")
            return
        prompt = """
